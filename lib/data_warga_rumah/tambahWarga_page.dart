...
class _TambahWargaPageState extends State<TambahWargaPage> {
  ...
  Future<void> _analyzeImage() async {
    ...
    setState(() => analyzing = true);
    try {
      final uri = Uri.parse('https://raki46-faceage-detection.hf.space/predict');
      final request = http.MultipartRequest('POST', uri);
      ...
      Map<String, dynamic>? result;
      if (decoded is Map && decoded.containsKey('confidence') && decoded.containsKey('predicted_age')) {
        result = Map<String, dynamic>.from(decoded);
      } else if (decoded is Map && decoded.containsKey('data')) {
        final d = decoded['data'];
        if (d is List && d.isNotEmpty && d[0] is Map) result = Map<String, dynamic>.from(d[0]);
      }

      if (result != null && result.containsKey('confidence') && result.containsKey('predicted_age')) {
        final confRaw = result['confidence'];
        final pred = result['predicted_age'];
        final confNum = (confRaw is num) ? confRaw.toDouble() : double.tryParse(confRaw.toString()) ?? 0.0;
        final confPct = '${(confNum * 100).toStringAsFixed(1)}%';
        if (!mounted) return;
        showDialog(
          context: context,
  ...
  InputDecoration _decoration(String hint) {
    return InputDecoration(
      hintText: hint,
      border: OutlineInputBorder(borderRadius: BorderRadius.circular(10)),
      enabledBorder: const OutlineInputBorder(borderSide: BorderSide(color: kombu)),
      focusedBorder: const OutlineInputBorder(borderSide: BorderSide(color: kombu, width: 2)),
    );
  }
              ...
              child: Column(
                children: [
                  ...
                  TextFormField(
                    ...
                    keyboardType: TextInputType.number,
                    decoration: _decoration("NIK"),
                    validator: (v) => v == null || v.length < 16 ? "NIK tidak valid" : null,
                  ),
                  const SizedBox(height: 16),
                  ...
                  Align(
                    alignment: Alignment.centerLeft,
                    child: Text('Foto Warga', style: TextStyle(fontWeight: FontWeight.w600, color: kombu)),
                  ),
                  const SizedBox(height: 8),
                  Row(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Container(
                        ...
                        child: _pickedImage == null
                            ? IconButton(
                                icon: const Icon(Icons.camera_alt, color: kombu),
                                onPressed: _pickImage,
                              )
                            : ClipRRect(
                                borderRadius: BorderRadius.circular(8),
                      ...
                      Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          OutlinedButton.icon(onPressed: _pickImage, icon: const Icon(Icons.upload_file), label: const Text('Pilih Foto')),
                          const SizedBox(height: 8),
                          ElevatedButton.icon(
                            onPressed: (_pickedImage == null || analyzing) ? null : _analyzeImage,
                            icon: analyzing
                  ...
                  Row(
                    ...
                    children: [
                      ElevatedButton(
                        onPressed: isLoading ? null : save,
                        style: ElevatedButton.styleFrom(backgroundColor: kombu, padding: const EdgeInsets.symmetric(horizontal: 28, vertical: 12)),
                        child: isLoading
                      ...
                      OutlinedButton(
                        ...
                        style: OutlinedButton.styleFrom(side: const BorderSide(color: kombu)),
